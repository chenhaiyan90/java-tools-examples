...
public class RsaUtils {
    ...
    public static byte[] encryptDataBlock(byte[] data, Key key) {
        ...
        try {
            ...
            while (readLen > 0) {
                ...
                byte[] encData = encryptData(dataToEnc, key);
                bos.write(encData);
                readLen = bis.read(buffer, 0, bufferSize);
            }
            result = bos.toByteArray();    //得到加密结果
            bis.close();
            bos.close();
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public static boolean verifySign(byte[] data, PublicKey key, String sign){
        try{
            Signature signature = Signature.getInstance(SIGNATURE_ALGHORITHEM);
